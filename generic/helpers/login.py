from dm_api_account.models import LoginCredentials


class Login:
    def __init__(self, facade):
        from services.dm_api_account import Facade
        self.facade: Facade = facade

    def set_headers(self, headers):
        self.facade.login_api.client.session.headers.update(headers)

    def login_new_user(self, login: str, password: str, rememberMe: bool = True):
        response = self.facade.login_api.post_v1_account_login(
            json=LoginCredentials(
                login=login,
                rememberMe=rememberMe,
                password=password

            )
        )
        return response

    def get_auth_token(self, login: str, password: str, rememberMe: bool = True):
        response = self.login_new_user(login=login, password=password, rememberMe=rememberMe)
        token = {'X-Dm-Auth-Token': response.headers['X-Dm-Auth-Token']}
        return token

    def logout_user(self, **kwargs):
        response = self.facade.login_api.delete_v1_account_login(**kwargs)
        return response

    def logout_user_from_all(self, **kwargs):
        response = self.facade.login_api.delete_v1_account_login_all(**kwargs)
        return response
